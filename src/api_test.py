...
class OnDemandTest(ApiTestBase):
  def test_resolve_url(self):
    ...
    response = self.app.get('/api/ondemand', params={'url': 'https://github.com/org/repo/pull/1'})
    self.assertEqual(response.status_int, 200)
    self.assertEqual(response.normal_body, 'pullsha')

    response = self.app.get('/api/ondemand', params={'url': 'https://github.com/org/repo/commit/commitsha'})
    self.assertEqual(response.status_int, 200)
    ...
    response = self.app.get('/api/ondemand', params={'url': 'https://github.com/org/repo/tree/branch'})
    self.assertEqual(response.status_int, 200)
    ...
    response = self.app.get('/api/ondemand', params={'url': 'https://github.com/org/repo'})
    self.assertEqual(response.status_int, 200)
    self.assertEqual(response.normal_body, 'mastersha')

    response = self.app.get('/api/ondemand', params={'url': 'https://github.com/org/repo/pull/1/commits/pullcommitsha'})
    self.assertEqual(response.status_int, 200)
